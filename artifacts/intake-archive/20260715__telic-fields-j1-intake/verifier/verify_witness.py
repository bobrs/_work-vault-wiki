...
def verify(export_zip: Path) -> dict[str, Any]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="telic-j1-independent-") as temp_name:
        root = Path(temp_name)
    ...
    return {
        ...
        "verifier":"standalone-no-telic-j1-import",
    }


def main() -> int:
    parser = argparse.ArgumentParser(description="Independent verifier for Telic Fields J.1 witness exports")

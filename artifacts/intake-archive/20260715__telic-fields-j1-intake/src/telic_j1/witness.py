...
def export_witness(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="telic-j1-export-") as temp_name:
        root = Path(temp_name)
...
def verify_export(export_zip: Path) -> dict[str, Any]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="telic-j1-verify-") as temp_name:
        root = Path(temp_name)
        ...
        for family in FAMILY_TO_SCHEMA:
            ...
            for record in json.loads(path.read_text(encoding="utf-8"))["records"]:
                try:
                    ...
                    validated_records += 1
                    object_id = record.get("id") or record.get("event_id") or record.get("context_id") or record.get("profile_id") or record.get("report_id") or record.get("transaction_id") or record.get("signature_id")
                    if object_id:

...
class PilotWebApp:
    def __init__(self, workdir: Path, schema_dir: Path):
        ...
        self.pilot = ReferencePilot(self.workdir, self.schema_dir)
...
<title>Telic Fields J.1 Pilot Hardening</title><style>{CSS}</style></head>
<body>
<a class="skip-link" href="#main">Skip to main content</a>
<header><h1>Telic Fields J.1</h1><p>Independent verification, selective disclosure, and pilot hardening.</p></header>
...
<form method="post" action="/step/full"><button type="submit">Run full hardened scenario</button></form>
<form method="post" action="/step/seed"><button type="submit">Seed records only</button></form>
<form method="post" action="/step/correct"><button type="submit">Apply participant correction and policy update</button></form>
...
<footer><p>Reference pilot only. No production personal data.</p></footer>
    ...
    def __call__(self, environ, start_response):
        ...
        if path == "/step/correct" and method == "POST":
            self.pilot.correct_participant_b_and_rotate_policy(); return self._redirect(start_response)
        ...
        if path == "/step/full" and method == "POST":
            self.pilot.run_full(self.workdir / "tf-mvi-1-j1-witness.zip"); return self._redirect(start_response)
...
def serve(workdir: Path, schema_dir: Path, host: str, port: int) -> None:
    ...
    with make_server(host, port, app) as server:
        print(f"Serving J.1 pilot at http://{host}:{port}")

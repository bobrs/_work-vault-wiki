...
class AuthorizationPolicyRegistry:
    ...
    def activate(self, version: int) -> dict[str, Any]:
        if version not in self._policies:
            raise PolicyVersionError(f"Unknown policy version: {version}")
        if self._active_version is not None and self._active_version in self._policies:
            self._policies[self._active_version]["status"] = "superseded"
        self._policies[version]["status"] = "active"
        self._active_version = version
        return dict(self._policies[version])

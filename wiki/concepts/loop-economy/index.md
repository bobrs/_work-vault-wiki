# Loop Economy

Working definition: a participation-based economy model expressed through loop units, feedback controls, and circulation mechanics rather than a conventional market frame.

This concept appears throughout the SCT `Loop Economy Series` and the surrounding economy and propagation pages, where the work treats value as something routed, redistributed, and scaled through loops.

In practice, Loop Economy is the branch’s language for participation under circulation pressure: exchange, escrow, reputation, redistribution, and bonding curves are all ways the system decides how value moves and how it responds to imbalance.

The useful part of the concept is that it translates economic behavior into a protocol vocabulary. That lets the corpus talk about value without reducing it to price, and talk about participation without reducing it to access. The loop frame keeps the movement visible: who contributes, who receives, what is retained, and what is re-routed.

Because of that, Loop Economy belongs near [LoopLink](../looplink/index.md), [PDSP](../pdsp/index.md), [Intent-Consent](../intent-consent/index.md), and [Governance Diad](../governance-diad/index.md). The economy only stays coherent if the loop, the consent surface, and the rule surface can all be named together.

## Related Artifacts

- [LE-Index — Foundations of a Loop-Based Economy.docx](<../../../artifacts/intake-archive/20260622__consent-crystal-structure-research-intake/Semantic Collapse Theory/Loop Series/Loop Economy Series/LE-Index — Foundations of a Loop-Based Economy.docx>)
- [LE-Index-Addendum I — The Wealth of Participation.docx](<../../../artifacts/intake-archive/20260622__consent-crystal-structure-research-intake/Semantic Collapse Theory/Loop Series/Loop Economy Series/LE-Index-Addendum I — The Wealth of Participation.docx>)
- [LE-Index-Addendum II — The Participation Economy.docx](<../../../artifacts/intake-archive/20260622__consent-crystal-structure-research-intake/Semantic Collapse Theory/Loop Series/Loop Economy Series/LE-Index-Addendum II — The Participation Economy.docx>)
- [LE-Loop 1 — The Reward Loop.docx](<../../../artifacts/intake-archive/20260622__consent-crystal-structure-research-intake/Semantic Collapse Theory/Loop Series/Loop Economy Series/LE-Loop 1 — The Reward Loop.docx>)
- [LE-Loop 5 — The Reputation Loop.docx](<../../../artifacts/intake-archive/20260622__consent-crystal-structure-research-intake/Semantic Collapse Theory/Loop Series/Loop Economy Series/LE-Loop 5 — The Reputation Loop.docx>)
- [SCT-33- Loop Economy vs. Hierarchical Optimization — A Structural Response to Centralized Fragility.docx](<../../../artifacts/intake-archive/20260622__consent-crystal-structure-research-intake/Semantic Collapse Theory/SCT-33- Loop Economy vs. Hierarchical Optimization — A Structural Response to Centralized Fragility.docx>)

## Related Pages

- [Semantic Collapse Theory](../../projects/semantic-collapse-theory/index.md)
- [Loop Economy Series](../../projects/semantic-collapse-theory/loop-series/loop-economy-series/index.md)
- [Foundations and Appendices](../../projects/semantic-collapse-theory/loop-series/loop-economy-series/foundations-and-appendices/index.md)
- [Loop Units](../../projects/semantic-collapse-theory/loop-series/loop-economy-series/loop-units/index.md)
- [Economy and Propagation](../../projects/semantic-collapse-theory/economy-and-propagation/index.md)
- [LoopLink](../looplink/index.md)
- [PDSP](../pdsp/index.md)
- [Intent-Consent](../intent-consent/index.md)
- [Governance Diad](../governance-diad/index.md)
- [POLEMEMELOP](../polememelop/index.md)

## Attractor Bridge

- [Loop Mechanics](../../attractors/loop-mechanics/index.md)
- [Governance](../../attractors/governance/index.md)

## Notes

- This is a concept page, not a canon claim.
- Loop Economy should remain separable from any one branch page.
- The concept spans both reference material and operational loop units.

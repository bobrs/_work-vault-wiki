# Continuity Engine

This branch holds the structural governance briefs that sit alongside `Continuity Office`. The material now splits cleanly into three durable seams: client-facing positioning, sovereign governance execution, and firm-specific pilot materials.

## Current Shape

- 7 published documents.
- 3 nested lineage pages organize the set.

## Nested Lineage Pages

- [Client Materials](client-materials/index.md)
- [Sovereign Governance](sovereign-governance/index.md)
- [Firm Materials](firm-materials/index.md)

## Representative Files

- [Continuity Engine Client FAQ.md](<../../../artifacts/standard-named/20260622__CONTINUITY-ENGINE__FAQ__CLIENT__v1__continuity-engine-client-faq.md>) · [archive copy](<../../../artifacts/intake-archive/20260622__continuity-engine-intake/continuity_engine_client_faq_v_1.md>)
- [Continuity Engine vs Typical Governance Tools.md](<../../../artifacts/standard-named/20260622__CONTINUITY-ENGINE__COMPARISON__GOVERNANCE-TOOLS__v1__continuity-engine-vs-typical-governance-tools.md>) · [archive copy](<../../../artifacts/intake-archive/20260622__continuity-engine-intake/continuity_engine_vs_typical_governance_tools_v_1.md>)
- [Continuity Engine Tribal Council Brief.md](<../../../artifacts/standard-named/20260622__CONTINUITY-ENGINE__BRIEF__TRIBAL-COUNCIL__v1__continuity-engine-tribal-council-brief.md>) · [archive copy](<../../../artifacts/intake-archive/20260622__continuity-engine-intake/continuity_engine_tribal_council_brief_v_1.md>)
- [Continuity Engine for Sovereign Governance and Tribal Gaming.md](<../../../artifacts/standard-named/20260622__CONTINUITY-ENGINE__BRIEF__SOVEREIGN-GOVERNANCE__v1__continuity-engine-for-sovereign-governance-tribal-gaming.md>) · [archive copy](<../../../artifacts/intake-archive/20260622__continuity-engine-intake/continuity_engine_for_sovereign_governance_tribal_gaming_v_1.md>)
- [Continuity Engine Executive Brief.md](<../../../artifacts/standard-named/20260623__CONTINUITY-ENGINE__BRIEF__EXECUTIVE__v1__continuity-engine-executive-brief.md>) · [archive copy](<../../../artifacts/intake-archive/20260623__continuity-engine-intake/continuity_engine_executive_brief_v_1.md>)

## Working Read

The client-facing material frames Continuity Engine as an execution layer for governed workflows: it makes procedures visible, enforces structure at runtime, and keeps AI usage inside defined guardrails.

The sovereign governance material uses the same structural language but applies it to Tribal law, commission rules, compact obligations, and regulatory review.

The firm-specific material turns the same structure toward a 90-day pilot, executive positioning, and partner-facing adoption language.

## Related Links

- [Continuity Office](../continuity-office/index.md)
- [Intent-Consent](../../concepts/intent-consent/index.md)
- [Witnessing](../../concepts/witnessing/index.md)

## Next Actions

1. Keep the client-facing and sovereign-governance seams separate.
2. Keep the firm-specific pilot material together as one branch unless a stronger audience seam appears.
3. Treat the dated duplicates and companion `.docx` files as archive material unless a stronger live branch appears.
4. Add more continuity-engine material only if it changes the structure, not just the wording.

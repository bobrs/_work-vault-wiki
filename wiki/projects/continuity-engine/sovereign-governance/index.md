# Sovereign Governance

Parent lineage: `Continuity Engine`

This seam holds the governance briefs aimed at Tribal council and Tribal gaming contexts. The material is about preserving authority hierarchy while making directives and reviews easier to see in real time.

## Current Shape

- 2 briefs.

## Representative Files

- [Continuity Engine Tribal Council Brief.md](<../../../../artifacts/standard-named/20260622__CONTINUITY-ENGINE__BRIEF__TRIBAL-COUNCIL__v1__continuity-engine-tribal-council-brief.md>) · [archive copy](<../../../../artifacts/intake-archive/20260622__continuity-engine-intake/continuity_engine_tribal_council_brief_v_1.md>)
- [Continuity Engine for Sovereign Governance and Tribal Gaming.md](<../../../../artifacts/standard-named/20260622__CONTINUITY-ENGINE__BRIEF__SOVEREIGN-GOVERNANCE__v1__continuity-engine-for-sovereign-governance-tribal-gaming.md>) · [archive copy](<../../../../artifacts/intake-archive/20260622__continuity-engine-intake/continuity_engine_for_sovereign_governance_tribal_gaming_v_1.md>)

## Working Read

This seam uses the same structural model as the client materials but shifts the center of gravity to sovereign authority, compact obligations, regulatory review, and change impact. It reads as a governance execution layer for authority-sensitive environments.
It also sits close to the Governance attractor because the real subject is not the document format but the shape of authorization, accountability, and review under a hierarchy that must remain legible while it moves.
That makes it adjacent to the consent/governance line of work as well: when authority is explicit, the system still has to preserve revocation, provenance, and witness. The Consentocracy Bridge names the same move from the external essay side.
It also has a concrete sibling in [Voting Machine](../../voting-machine/index.md), where the same governance concerns appear as election custody, offline verification, and privacy-preserving audit rather than executive or council briefing.

## Related Links

- [Continuity Engine](../index.md)
- [Continuity Office](../../continuity-office/index.md)
- [Voting Machine](../../voting-machine/index.md)
- [Governance](../../../attractors/governance/index.md)
- [Trust](../../../attractors/trust/index.md)
- [Consent](../../../attractors/consent/index.md)
- [The Consentocracy Bridge](../../../external/shimmerymemory/essays/changing-the-optimization-primitive/index.md)

## Next Actions

1. Keep sovereign authority distinctions explicit.
2. Treat any future Tribal or gaming material as a continuation of this seam unless it clearly belongs elsewhere.

# Investor Materials

Parent lineage: `Advanced` / `Wold Building` / `Governance and Investor Materials`

This cluster holds the investor-facing material.

## Current Shape

- 2 documents.

## Representative Files

- [Consentocratic Investor Page.docx](<../../../../../../../../../artifacts/standard-named/20260622__SIDE-PROJECTS-DESKTOP__PAGE__WOLD-BUILDING__v1__consentocratic-investor-page.docx>) · [archive copy](<../../../../../../../../../artifacts/intake-archive/20260622__loop-and-phase-training-wold-building-intake/Consentocratic Investor Page.docx>)
- [Consentocratic Investor Pledge.docx](<../../../../../../../../../artifacts/standard-named/20260622__SIDE-PROJECTS-DESKTOP__PLEDGE__WOLD-BUILDING__v1__consentocratic-investor-pledge.docx>) · [archive copy](<../../../../../../../../../artifacts/intake-archive/20260622__loop-and-phase-training-wold-building-intake/Consentocratic Investor Pledge.docx>)

## Working Read

This page holds the external-facing investor surface of the Wold Building branch. It carries the pitch and pledge material that would be read by someone engaging the project from outside.

The two documents belong together because they describe the invitation and the response path. That keeps the investor side readable as a compact outward layer rather than mixing it with governance mechanics.

## Related Links

- [Governance and Investor Materials](../index.md)
- [Governance and Participation](../governance-and-participation/index.md)
- [Consent Wave and Self State](../../consent-wave-and-self-state/index.md)

## Next Actions

1. Keep the investor set stable.
2. Split only if a third investor-facing seam appears.
